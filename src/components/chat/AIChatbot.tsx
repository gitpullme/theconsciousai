...
import { History, Plus, X, Trash2, Send, Menu, MessageSquare, User, Bot } from 'lucide-react';
...
export default function AIChatbot({ isOpen, onClose, patientId = "user123" }: AIChatbotProps) {
  ...
  }, [fetchUserProfile, isOpen, session?.user?.id]);
  ...
  }, [isOpen, patientId, session?.user?.id, userProfile]);
  ...
  return (
    ...
    >
      {/* Chatbot modal container - prevent clicks from closing */}
      <motion.div 
        ...
        className="relative bg-white rounded-xl shadow-2xl overflow-hidden max-w-4xl w-full max-h-[85vh] h-[600px]"
      ...
      >
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.9 }}
          onClick={onClose}
          className="absolute top-3 right-3 z-10 p-2 text-gray-400 hover:text-gray-600 bg-white/80 hover:bg-gray-100 rounded-full transition-colors"
        >
          <X className="h-5 w-5" />
        </motion.button>
          ...
          <AnimatePresence>
            {showHistory && (
              <motion.div
                ...
                className="w-64 bg-gray-50 border-r border-gray-200 h-full"
              >
        <div className="p-4">
          <button
            onClick={startNewChat}
                    className="w-full flex items-center justify-center space-x-2 px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors shadow-sm"
          >
          ...
          {savedConversations.length > 0 && (
            ...
            >
              <Trash2 className="h-4 w-4" />
                ...
                <div className="overflow-y-auto h-[calc(100%-8rem)] px-2 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
                  <AnimatePresence>
          {savedConversations.length === 0 ? (
                      <motion.p 
                        initial={{ opacity: 0 }}
              ...
              {savedConversations.map(conversation => (
                  ...
                  key={conversation.id}
                            ...
                            className="group flex items-center justify-between p-2 rounded-md cursor-pointer overflow-hidden"
                >
                  <div 
      ...
      {/* Main chat area */}
          ...
          >
            ...
            <h2 className="text-lg font-semibold text-gray-900">AI Medical Assistant</h2>
                ...
                onClick={toggleFullHistory}
                        className="text-xs px-2 py-1 bg-indigo-50 hover:bg-indigo-100 rounded-md text-indigo-600 transition-colors"
              ...
              >
                ...
                onClick={toggleFullHistory}
                        className="text-xs px-2 py-1 bg-indigo-50 hover:bg-indigo-100 rounded-md text-indigo-600 transition-colors"
                title="Show recent messages only"
              >
                Show Recent
                      </motion.button>
                    )}
        ...
        {/* Messages */}
            <div className="flex-1 overflow-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
            ...
            >
                    ...
                    {message.role === 'assistant' && (
                      <motion.div 
                        ...
                        className="h-8 w-8 rounded-full bg-indigo-100 flex items-center justify-center mr-2"
                      >
          ...
          {isLoading && (
                  ...
                  >
                    ...
                    <div className="max-w-[75%] rounded-xl px-5 py-4 bg-white text-gray-900 border border-gray-100 shadow-sm rounded-tl-none">
          ...
          <div ref={messagesEndRef} />
            ...
            >
              <div className="flex items-center space-x-3 bg-gray-50 rounded-full px-4 py-2 shadow-sm border border-gray-200 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500 transition-all duration-200">
            <input
              type="text"
              value={input}
